...
public class ThumbnailsHandout extends DrawingGUI {
    ...
    private ArrayList<BufferedImage> thumbs;		// thumbnail images to display, a promise that you're going to have a list
    //called 'thumbs'

    public ThumbnailsHandout(ArrayList<BufferedImage> images) {
        initWindow(thumbWidth*tcols, thumbHeight*trows);
        thumbs = new ArrayList<BufferedImage>(); //allocate actual memory for thumbs
    ...
    private static BufferedImage thumbnailify(BufferedImage image) {
        ...
        for(int i=0;i<thumbWidth;i++){
            ...
            result.setRGB(i,j,image.getRGB(i*(image.getWidth()/result.getWidth()),j*(image.getHeight()/result.getHeight())));
            }
