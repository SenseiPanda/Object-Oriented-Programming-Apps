...
public class DotTreeGUI extends DrawingGUI {
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			// TODO: YOUR CODE HERE
			//if tree exists, insert into tree
			if (tree !=null){
				tree.insert(new Dot(x,y));
			}
			//otherwise make a new tree and insert
			else {
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		// 1. Set the color for this level
		g.setColor(rainbow[level % rainbow.length]);
		// TODO: YOUR CODE HERE
		System.out.println("Initial point is:"+tree.getPoint());
		//System.out.println("Y1 value: "+tree.getY1());
		//System.out.println("X1 value: "+tree.getX1());
		// 2. g.drawCircle for drawing the point, and start drawing the point minus radius for x and y (top left corner)
		g.fillOval( (int) (tree.getPoint().getX()-dotRadius), (int) tree.getPoint().getY()-dotRadius, dotRadius,dotRadius);
		// 3. Draw lines through it
		//draw the vertical line
		g.drawLine((int) tree.getPoint().getX()-dotRadius/2,tree.getY1(), (int) tree.getPoint().getX()-dotRadius/2, tree.getY2());
		//horizontal line
		g.drawLine(tree.getX1(), (int) tree.getPoint().getY()-dotRadius/2, tree.getX2(), (int) tree.getPoint().getY()-dotRadius/2);
		...
		if (tree.hasChild(1)){
			...
			System.out.println("Child 1 is: "+tree.getChild(1));
		}
		if (tree.hasChild(2)){
	...
	//let's get this f***ing party started
