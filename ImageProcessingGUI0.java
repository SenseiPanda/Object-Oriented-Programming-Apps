...
public class ImageProcessingGUI0 extends DrawingGUI {
    ...
    boolean brushDown = false;// how to interpret mouse press (taken from ImageProcessingGUI by Pierson)
    boolean mousePressed = false; // will be used when mouse is clicked and dragged across screen
    ...
    public void handleKeyPress(char op) {
        ...
        //declare and set original value of brush down to false
// put the brush down if the user presses the 'g' key
        else if (op=='g'){
            brushDown= true;
            System.out.println("Brush is down");
        }
        else if (brushDown=true){
            if (op=='u'){
                brushDown=false;
                System.out.println("Brush is not down anymore");
            }
        }
        else {
    ...
    public void handleMousePress(int x, int y) {
        ...
        mousePressed = true;
        if (brushDown==true) {
            proc.addGreen(x, y, 10);
        }
        repaint(); // Re-draw everything, since image has changed
    }
    @Override
    public void handleMouseMotion(int x, int y) {
        //supports a method called "handleMouseMotion"
        // that works exactly like "handleMousePress", but is called as the mouse moves.
        //if mouse press, perform the method getGreen
